...
import { Send, StopCircle, Loader, Bot, User, Sparkles, Mic, MicOff, Volume2, VolumeX, Phone, PhoneOff, ArrowLeft } from 'lucide-react';
...
const FreeConversation = () => {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: Date.now() + 1, role: 'assistant', content: aiMessage.content }]);
  ...
  const endConversation = async () => {
    ...
    try {
      ...
      setReportCard(response.data.data.reportCard);
      setShowReport(true);
    } catch (err) {
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Free Conversation</h1>
              <p className="text-sm text-gray-500">Natural conversation practice with AI</p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
        ...
        {isStarting ? (
          ...
          <>
            {messages.length <= 1 && (
              <div className="bg-emerald-50 rounded-xl p-4 mb-4 border border-emerald-100">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <p className="text-sm text-gray-700"><strong>Tips for better practice:</strong></p>
                    <ul className="text-sm text-gray-600 mt-1 space-y-1">
                      <li>• Toggle <strong>Voice On</strong> for hands-free voice conversation</li>
                      <li>• In voice mode the AI will reply aloud and auto-listen for your next turn</li>
                      <li>• Don't worry about mistakes — that's how you learn!</li>
                      <li>• Click <strong>End</strong> when you're done to see your report card</li>
                    </ul>
                  </div>
                </div>
              </div>
            ...
            {messages.map((message) => (
              <div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'} chat-message`}>
                <div className={`flex items-start space-x-3 max-w-[80%] ${message.role === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${message.role === 'user' ? 'bg-primary-500' : 'bg-emerald-600'}`}>
                  ...
                  <div className={`rounded-2xl px-4 py-3 ${message.role === 'user' ? 'bg-primary-500 text-white rounded-tr-sm' : 'bg-white text-gray-800 shadow-sm border border-gray-100 rounded-tl-sm'}`}>
                    ...
                    {message.role === 'assistant' && ttsSupported && (
                      <button
                        ...
                        className="mt-2 flex items-center space-x-1 text-xs text-gray-400 hover:text-emerald-500 transition-colors"
                      >
        ...
        {voiceMode ? (
          <div className="flex flex-col items-center py-2">
            <p className="text-sm text-gray-500 mb-3">
              {isSpeaking ? '🔊 AI is speaking...' : isLoading ? '🤔 AI is thinking...' : isListening ? '🎙️ Listening... speak now' : 'Tap the mic to speak'}
            </p>
            <button
              type="button"
              ...
              className={`flex items-center justify-center w-20 h-20 rounded-full transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-offset-2 disabled:opacity-50 ${
                isListening
                  ...
                  : 'bg-emerald-600 text-white hover:shadow-xl hover:scale-105 focus:ring-emerald-300 shadow-lg'
              }`}
            >
              {isListening ? (
          ...
          <>
            <form onSubmit={sendMessage} className="flex items-center space-x-3">
              <input
                ...
                placeholder={isListening ? '🎙️ Listening...' : 'Type or tap the mic to speak...'}
